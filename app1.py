...
def generate_story_huggingface_free(title: str, limit: int) -> str:
    """Free serverless inference using Hugging Face router API."""
...
def generate_gemini_story(title: str, limit: int) -> str:
    ...
    hf_story = generate_story_huggingface_free(title, limit)
    if hf_story:
        return hf_story

    # Tier 4: Dynamic Story Fallback
    return (
        f"Once upon a time, the legend of '{title}' unfolded across a forgotten world. "
        f"The hero embarked on a quest filled with unknown challenges, discovering hidden strengths "
        f"and unraveling mysteries that changed their destiny forever."
    )


def get_free_photo_url(title: str) -> str:
    """Uses direct image search artwork fallback instead of Pollinations."""
